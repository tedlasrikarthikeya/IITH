...
def graph():
    ...
    for i in range(len(accuracy)):  # Prevents index out-of-range errors
    ...
    for i in range(len(accuracy)):
        df_data.extend([
            [algorithms[i], "Precision", precision[i]],
            [algorithms[i], "Recall", recall[i]],
            [algorithms[i], "F1 Score", fscore[i]],
            [algorithms[i], "Accuracy", accuracy[i]],
        ])
